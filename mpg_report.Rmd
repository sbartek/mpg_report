...
```{r WTGimpact, fig.margin = TRUE, fig.cap = "Impact of weight, transmission and number of gears on fuel consumption."}
...
```{r} 
nvmax <- 16
regfit <- regsubsets(mpg~. ,cs.dt, nvmax =nvmax)
...
ggplot(data=NULL, aes(x=1:16, y=reg.summary$rsq)) + geom_line() +
    theme_bw()
